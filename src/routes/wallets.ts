 ...
 * the latest cached rate, with a grand total. Monero wallets are view-only
 * and cannot detect spends, so their value reflects lifetime received, not a
 ...
 */
wallets.get("/value", requireAuth, perKeyRateLimit, async (c) => {
  return c.json(await getWalletBalances(c.get("personalKeyId")));
});
